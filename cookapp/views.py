...
class MealPlanDeleteView(DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        # Check if the user is the owner of the meal plan
        if mealplan.user != request.user:
            messages.error(request, "You don't have permission to delete this meal plan.")
            return redirect('cookapp:mealplan_detail', mealplan_pk=mealplan.pk)
        
        # If user is the owner, proceed with deletion
        messages.success(request, f"Meal plan '{mealplan.name}' has been deleted.")
...
class DirectPasswordResetForm(forms.Form):
    ...
    def clean(self):
        ...
        if not user:
            ...
            self.add_error('email', "Email does not match the provided username.")
        
        return cleaned_data

@method_decorator(login_not_required, name='dispatch')
class DirectPasswordResetView(FormView):
    template_name = 'registration/direct_password_reset.html'
    ...
    def form_valid(self, form):
        ...
        if user:
            ...
            # Add success message
            messages.success(self.request, "Your password has been changed successfully. Please log in with your new password.")
            return super().form_valid(form)
